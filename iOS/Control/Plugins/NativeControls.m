...
{
	//NSLog(@"creating tab bar %@", [arguments description]);
    tabBar = [UITabBar new];
    [tabBar sizeToFit];
...
- (void)showTabBar:(NSArray*)arguments withDict:(NSDictionary*)options
{
    ...
    if (options) 
	{
		...
		if([options objectForKey:@"orientation"] != nil) {
			isPortrait = [[options objectForKey:@"orientation"] isEqualToString:@"portrait"];
		}else{
			isPortrait = NO;
		}
    }
	...
	if (atBottom) 
	{
		...
		if(isPortrait) {
			...
			tabBarBounds = CGRectMake(
				...
				webViewBounds.origin.x + webViewBounds.size.width - height - offsetRect.size.width,
