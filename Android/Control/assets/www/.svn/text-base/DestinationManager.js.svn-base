function DestinationManager() {
	...
	this.addDestination = function(address, port, isBonjour, isMIDI) {
		...
		if(isBonjour) {
			item.innerHTML = "<div style='display:inline'>" + address + ":" + port +  "</div><img style='margin-top:.15em; width:2.5em; height:2.5em; float:right' src='bonjour.png'>";
		}else if(isMIDI) {
			item.innerHTML = "<div style='display:inline'>" + address + ":" + port +  "</div><img style='margin-top:.15em; width:2.5em; height:2.5em; float:right' src='MIDI_Icon.png'>";
		}else{
